...
public class Colors {
  ...
  public static function adjustHsv(color:uint, h:Number = 0, s:Number = 0, v:Number = 0, a:uint = 255):uint {
    var ch:Number = clamp((hue(color) + h) % 1); // the hue goes round and round
    var cs:Number = clamp(saturation(color) + s);
    var cv:Number = clamp(value(color) + v);
  ...
  public static function setHsv(color:uint, h:Number = NaN, s:Number = NaN, v:Number = NaN, a:uint = 255):uint {
    ...
    const cs:Number = isNaN(s) ? saturation(color) : clamp(s);
    const cv:Number = isNaN(v) ? value(color) : clamp(v);
    return hsv(ch, cs, cv, a);
  }
  ...
  /**
   * Restricts a number to between minval and maxval
   *
   ...
   *
   * @param cssSelector a string indicating the css selector, e.g., "Button", ".myStyle"
   * @param cssProperty the css property to change, e.g., "color", "backgroundColor"
  ...
  public static function adjustStyle(cssSelector:String, cssProperty:String, colorAdjustmentFunction:Function):void {
    ...
    if (styleDecl != null) {
      ...
      if (colorProp is uint) {
        styleDecl.setStyle(cssProperty, colorAdjustmentFunction(colorProp));
      }
      if (colorProp is Array) {
        var newArray:Array = [];
        for each (var c:uint in colorProp) {
          newArray.push(colorAdjustmentFunction(c));
        }
        styleDecl.setStyle(cssProperty, newArray);
      }
    }
  }
